...
impl Solution for BoilingBoulders {
    fn one(self) -> i64 {
        self.faces()
            .values()
            .map(|face| face.count_ones())
            .sum::<u32>() as i64
    }

    fn two(self) -> i64 {
        let faces = self.faces();
        let (min @ (min_x, min_y, min_z), (max_x, max_y, max_z)) = self.bounds();
...
impl BoilingBoulders {
    fn faces(&self) -> HashMap<(i64, i64, i64), u8> {
        let mut faces = HashMap::new();

        for (x, y, z) in self.0.iter().copied() {
    ...
    fn bounds(&self) -> ((i64, i64, i64), (i64, i64, i64)) {
